...
class OrderController extends Controller
{
    ...
    public function returnpay(){
        ...
        */
        if($result) {//验证成功
            ...
            if($count){
                echo "支付宝交易号：".$trade_no."订单号：".$where['order_no']."订单金额".$where['order_acount']."此订单有问题";exit;
            }
            if(config('pay.seller_id')!=htmlspecialchars($_GET['seller_id'])){
                echo "支付宝交易号：".$trade_no."订单号：".$where['order_no']."订单金额".$where['order_acount']."此订单有问题，商户id不匹配";exit;
            };
            if(config('pay.app_id')!=htmlspecialchars($_GET['app_id'])){
                echo "支付宝交易号：".$trade_no."订单号：".$where['order_no']."订单金额".$where['order_acount']."此订单有问题，app_id不匹配";exit;
            };    
    ...
    //支付 mb端支付
    public function mbalipay($id){
        $orderInfo = DB::table('order')->where('order_id',$id)->first();
        if(empty($orderInfo)){
            echo "订单信息错误";
        }

        require_once app_path('libs/mbalipay/moalipay/wappay/service/AlipayTradeService.php');
        require_once app_path('libs/mbalipay/moalipay/wappay/buildermodel/AlipayTradeWapPayContentBuilder.php');
        $config = config('pay');
            ...
            $result=$payResponse->wapPay($payRequestBuilder,config('pay.return_url'),config('pay.notify_url'));
    ...
    public function notifypay(){
        ...
        */
        if($result) {//验证成功
            ...
            $count=DB::table('order')->where($where)->count();
            if($count){
                \Log::info("支付宝交易号：".$trade_no."订单号：".$where['order_no']."订单金额".$where['order_acount']."此订单有问题");exit;
               
            }
            if(config('pay.seller_id')!=htmlspecialchars($_POST['seller_id'])){
                \Log::info("支付宝交易号：".$trade_no."订单号：".$where['order_no']."订单金额".$where['order_acount']."此订单有问题，商户id不匹配");exit;
                // "支付宝交易号：".$trade_no."订单号：".$where['order_no']."订单金额".$where['order_acount']."此订单有问题，商户id不匹配";exit;
            };
            if(config('pay.app_id')!=htmlspecialchars($_POST['app_id'])){
                echo "支付宝交易号：".$trade_no."订单号：".$where['order_no']."订单金额".$where['order_acount']."此订单有问题，app_id不匹配";exit;
            };    
            // echo "验证成功<br />支付宝交易号：".$trade_no;
            \Log::info("验证成功<br />支付宝交易号：".$trade_no);
